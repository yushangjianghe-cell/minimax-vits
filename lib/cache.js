...
class AudioCacheManager {
    constructor(root, logger, options = {}) {
        ...
        this.indexFile = path_1.default.join(root, 'index.json');
        this.options = {
    ...
    async initialize() {
        ...
        try {
            ...
            if (fs_1.default.existsSync(this.indexFile)) {
                const data = await fs_1.default.promises.readFile(this.indexFile, 'utf-8');
    ...
    async getAudio(text, voiceId, params) {
        ...
        if (entry) {
            ...
            try {
                const buffer = await fs_1.default.promises.readFile(entry.filePath);
    ...
    async saveAudio(text, voiceId, params, buffer) {
        ...
        const filePath = path_1.default.join(this.root, fileName);
        try {
        ...
        if (entry) {
            try {
                if (fs_1.default.existsSync(entry.filePath)) {
                    await fs_1.default.promises.unlink(entry.filePath);
                }
            }
            catch (e) {
                // ignore
    ...
    async saveIndex() {
        try {
            ...
            await fs_1.default.promises.writeFile(this.indexFile, data);
        }
        catch (e) {
